        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            ...
            {application.status?.toLowerCase() === 'disbursed' && (
              <Link
                href={`/operator/post-disbursement?applicationId=${application.id}`}
            ...
            <div className="flex justify-between items-center space-x-4">
              ...
              <div>
                ...
                {application.marketplace_status && (
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${application.marketplace_status === 'not_distributed' ? 'bg-gray-100 text-gray-800' :
                        ...
                        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                          ...
                          <div>
                            ...
                            {doc.verification_status !== 'verified' && doc.verification_status !== 'rejected' ? (
                              ...
                              <div className={`p-4 rounded-md ${doc.verification_status === 'verified' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
                                }`}>
                                <p className={`text-sm font-medium ${doc.verification_status === 'verified' ? 'text-green-800' : 'text-red-800'
                                  }`}>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {applicationData.distributions.map((distribution) => (
                          <tr key={distribution.id} className="hover:bg-gray-50">
                            ...
                            {/* Status */}
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${distribution.status === 'offer_received' ? 'bg-green-100 text-green-800' :
            ...
            <div className="space-y-6">
              {offers.length > 0 ? (
                <div className="space-y-4">
                  ...
                  {offers.map(offer => (
                    <div key={offer.id} className={`bg-white shadow rounded-lg border-2 ${offer.id === application.selected_offer_id ? 'border-green-500 bg-green-50' : 'border-gray-200'
                      }`}>
                      <div className="p-6">
