...
export default function PostDisbursementPage() {
    ...
    useEffect(() => {
        if (applicationIdParam && cases.length > 0) {
            const foundCase = cases.find(c => c.id === parseInt(applicationIdParam))
            if (foundCase) {
    ...
    const fetchDocumentTypes = async () => {
        try {
            ...
            const data = await res.json()
            if (data.success) {
                // Filter for PDD documents (is_pdd = 1)
                const pddTypes = data.data.filter(t => t.is_pdd)
                setDocumentTypes(pddTypes)
            }
        } catch (error) {
    ...
    const handleUpload = async (e) => {
        ...
        formData.append('loanApplicationId', selectedCase.id)
        formData.append('customerId', selectedCase.customer_id)
                        ...
                        <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                            ...
                            <form onSubmit={handleUpdateCase} className="space-y-4">
                                ...
                                <div className="border-t border-gray-200 pt-4 mt-4">
                                    <h4 className="text-sm font-medium text-gray-900 mb-3">Upload Documents (RC, Insurance, etc.)</h4>
                                    <div className="flex items-end gap-3 bg-gray-50 p-3 rounded border border-gray-200">
                                        <div className="flex-1">
                                            <label className="block text-xs font-medium text-gray-700 mb-1">Document Type</label>
                                            <select
                                                value={selectedDocType}
                                                onChange={e => setSelectedDocType(e.target.value)}
                                                className="block w-full text-sm border-gray-300 rounded-md shadow-sm p-2 border text-gray-900"
                                            >
                                        ...
                                        <button
                                            ...
                                            disabled={uploading || !file || !selectedDocType}
                                            className="px-3 py-2 bg-indigo-600 text-white text-sm font-medium rounded hover:bg-indigo-700 disabled:opacity-50"
                                        >
                                                ...
                                                <div key={index} className="p-3 flex justify-between items-center">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500">
                                                            {doc.verification_status} • {new Date(doc.uploaded_at || Date.now()).toLocaleDateString()}
                                                        </p>
                                                    </div>
                                                    <a
                                                        href={doc.file_path}
